...
class RouterMultiHeadAttention(Module):
    ...
    def forward(self, query, key, value, average_attn_weights):  # mimic the input of MultiheadAttention
...
class NoRouterMultiHeadAttention(Module):
    # for context-points only (self-attention), can't be used for cross-attention between context-points and test points
    def __init__(self, d_model, nhead, dropout, batch_first, device, dtype, d_ff=None, num_R=50,
                 dropout_rate=0.2,
                 **kwargs):
        super(NoRouterMultiHeadAttention, self).__init__()
        factory_kwargs = {'device': device, 'dtype': dtype}
        self.batch_first = batch_first

        # No router, only cross-attention
        self.dropout = nn.Dropout(dropout)
        self.ln1 = nn.LayerNorm(d_model)

    def forward(self, query, key, value, average_attn_weights):
        rep = self.ln1(query + self.dropout(query))
        return rep, {}
...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, 
        ...
        if self.num_R is not None:  # and not (self.last_layer_no_R and self.is_final_layer):  # use RouterMHA
            ...
            # when no zero init + router -> RouterMuliHeadAttention
            ...
            # when zero init <-> no router -> NoRouterMuliHeadAttention
            self.router_att = NoRouterMultiHeadAttention(d_model, nhead, dropout=dropout, batch_first=batch_first,
                                                       num_R=self.num_R, **factory_kwargs)
        ...
        # commenting to run attention only
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None,
        ...
        # Commented to run attention only model
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src_))))
        src = src + self.dropout2(src2)
        if not self.pre_norm:
           src = self.norm2(src)
