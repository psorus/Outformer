...
def load_model_workflow(i, e, add_name, base_path, device='cpu', eval_addition='', only_inference=True):
    """
    Workflow for loading a prior and setting appropriate parameters for diffable hparam tuning.
    ...
    # if only_inference:
    #     # print('Loading prior that can be used for inference only')
    #     prior, c = load_model_only_inference(base_path, model_file, device)
    # else:
    #     #until now also only capable of inference
    #     prior, c = load_model(base_path, model_file, device, eval_positions=[], verbose=False)
    # #prior, c = load_model(base_path, model_file, device, eval_positions=[], verbose=False)
...
class TabPFNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, device='cpu', base_path=pathlib.Path(__file__).parent.parent.resolve(), model_string='',
        ...
        """
        Initializes the classifier and loads the prior.
        Depending on the arguments, the prior is either loaded from memory, from a file, or downloaded from the
        repository if no prior is found.
        ...
        :param device: If the prior should run on cuda or cpu.
        :param base_path: Base path of the directory, from which the folders like models_diff can be accessed.
        :param model_string: Name of the prior. Used first to check if the prior is already in memory, and if not,
               tries to load a prior with that name from the models_diff directory. It looks for files named as
        ...
        :param feature_shift_decoder: If set to true shifts the features for each ensemble configuration according to a 
               random permutation.
        :param only_inference: Indicates if the prior should be loaded to only restore inference capabilities or also
...
def transformer_predict(model, eval_xs, eval_ys, eval_position,
    ...
    def predict(eval_xs, eval_ys, used_style, softmax_temperature, return_logits):
        ...
        with inference_mode_call:
            ...
            # else:
            #    output[:, :, 1] = prior((style.repeat(eval_xs.shape[1], 1) if style is not None else None, eval_xs, eval_ys.float()),
    ...
    def preprocess_input(eval_xs, preprocess_transform):
        import warnings
        # print('inside preprocessing_input..........')
        # print(eval_xs.shape)
