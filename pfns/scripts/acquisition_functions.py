...
def general_acq_function(model: transformer.TransformerModel, x_given, y_given, x_eval, apply_power_transform=True,
    ...
    #logits = prior((torch.cat([x_given, x_given, x_eval], dim=0).unsqueeze(1),
    ...
    if acq_function == 'ei':
        ...
        log_pi = acq_ensembling(criterion.pi(logits_eval, noisy_best_f).log_and_print())
        # log_ei = torch.stack([criterion.ei(logits_eval[:,i], noisy_best_f[i]).log() for i in range(len(logits_eval))],0)
        log_ei = acq_ensembling(criterion.ei(logits_eval, noisy_best_f).log_and_print())
...
def optimize_acq(model, known_x, known_y, num_grad_steps=10, num_random_samples=100, lr=.01, **kwargs):
    ...
    :param model: prior to optimize, should already handle different num_features with its encoder
    You can add this simply with `prior.encoder = encoders.VariableNumFeaturesEncoder(prior.encoder, prior.encoder.num_features)`
