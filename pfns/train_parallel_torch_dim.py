...
class MetricRecorder:
    ...
    def fetch_and_print(self, epoch=None, lr=None):
        ...
        # print(self.gmm_step_count)
        # print(self.copula_step_count)
        # print(self.scm_contextual_step_count)
        # print(self.scm_prob_step_count)
        # print(self.gmm_loss)
        # print(self.copula_loss)
        # print(self.scm_contextual_loss)
        # print(self.scm_prob_loss)
...
class ZeroShotOD(pl.LightningModule):
    def __init__(self, 
        ...
        # self.intial_assigned_weights = {}
        # for i in self.model_names_list:
        #     self.intial_assigned_weights[i] = 0.25
        # self.data_weights = [0.25 for i in self.model_names_list]
        # self.data_weights_map = dict(zip(self.model_names_list, self.data_weights))
        
        print(self.data_weights)
        print(self.data_weights_map)
        ...
        self.T0 = T0
        self.eta = 5 #learning rate for weights updates
    ...
    def on_train_epoch_start(self) -> None:
        ...
        #     if self.global_rank == 0:
        #         rewards = self._calculate_loss() #self._calculate_gradient_norms() #self._calculate_loss() #
        ...
        #         # print(rewards_np)
        #         # Convert losses -> rewards (rank-based). Smallest loss -> reward 1, largest -> 0.
                
        #         losses_np = np.array([rewards[key] for key in sorted(rewards.keys())], dtype=float)
        #         order = np.argsort(losses_np)              # ascending (best first)
        #         ranks = np.empty_like(order)
        #         ranks[order] = np.arange(len(losses_np))   # 0 = best
        #         rewards_np = 1.0 - ranks / (len(losses_np) - 1 + 1e-8)
        #         print(rewards_np)
        ...
        #         w_new /= w_new.sum()
        #         print('current_w_new', w_new)
        #         u = np.array([0.25,0.25,0.25,0.25])
        #         # get vt
        #         v = (1 - alpha) * w_new  + alpha * u
    ...
    def _calculate_loss(self):
        """
        Lightweight per-model-type loss calculation.
        """
        ...
        for batch in dl:
            ...
            # Mean over time / sequence dimension -> shape (N_priors,)
            col_means = torch.nanmean(losses_detached, dim=0)
            # Ensure names is a list
            names_list = list(names)
            assert col_means.numel() == len(names_list), f"Mismatch: {col_means.shape} vs {len(names_list)}"
            for i, name in enumerate(names_list):
                if name in accum:
                    accum[name].append(col_means[i].item())
        # Average
        result = {}
        for k, vals in accum.items():
            if vals:
                result[k] = sum(vals)/len(vals)
                print(f"[Rank {self.global_rank}] Loss {k}: {result[k]:.6f}")
        # Cleanup
