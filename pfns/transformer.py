...
#from .fomo_layer import TransformerEncoderLayer, _get_activation_fn
...
class TransformerModel(nn.Module):
    def __init__(self, encoder, ninp, nhead, nhid, nlayers, dropout=0.0, style_encoder=None, y_encoder=None,
                 ...
                 all_layers_same_init=False, efficient_eval_masking=True, decoder_once_dict=None,
        ...
        def make_decoder_dict(decoder_description_dict):
            ...
            for decoder_key in decoder_description_dict:
                ...
                if decoder_model is None:
                    initialized_decoder_dict[decoder_key] = nn.Sequential(nn.Linear(ninp, nhid),
                                                                          nn.GELU(),
        ...
        self.decoder_dict_once_embeddings = torch.nn.Parameter(
            torch.randn((len(self.decoder_dict_once), 1, ninp))) if self.decoder_dict_once is not None else None
    ...
    def init_weights(self):
        ...
        # commented for no router (no zero init)
        '''
        ...
        #Commented to turn off zero init to run attention only architecture (Saicharan Emmadi)
    ...
    def _forward(self, src, src_mask=None, single_eval_pos=None, only_return_standard_out=True):
        ...
        # print('in _forward before encoder')
        # print(x_src.shape)
        x_src = self.encoder(x_src)  # embedding layer
        # print('in _forward after encoder')
        # print(x_src.shape)
        self.after_embedding = x_src
        ...
        if src_mask is None:
            # print('scr mask is None.............')
            # print(self.global_att_embeddings)
        ...
        if y_src is not None:
            # print('adding y_src to train_x is called..............')
            train_x = train_x + y_src[:single_eval_pos]
        # print('constructing src')
        # print(global_src, style_src)
        # print(x_src.shape)
        # print(train_x.shape, x_src[single_eval_pos:].shape)
        ...
        # print('inside transformer _forward for src, src_mask')
        # print(src.shape, src_mask)
        output = self.transformer_encoder(src, src_mask)
        # print('inside transformer _forward for output')
        # print(output.shape)
        ...
        # print('in Transformer _forward, before return')
        # print(output['standard'].shape)
        # print(out_range_start, out_range_end)
        # print(self.decoder_dict)
        if only_return_standard_out:
            return output['standard']
        #output['standard'] = nn.Sigmoid(output['standard'])
        if output_once:
            print('output once called')
    ...
    def init_from_small_model(self, small_model):
        ...
        for my_layer, small_layer in zip(self.transformer_encoder.layers, small_model.transformer_encoder.layers):
            ...
            # commented to run attention only
