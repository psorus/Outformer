...
class Warp(gpytorch.Module):
    ...
    def forward(self, X: Tensor) -> Tensor:
        r"""Transform the inputs to a prior.
        ...
        if sample_from_path:
            ...
            for parameter_name, parameter_values in parameter_sample_distribution.items():
                ...
                try:
                    ...
                    utils.print_once(f'could not find parameter {parameter_name} in prior for `sample_from_extra_prior`')
        ...
        for i in range(0, total_num_candidates, num_candidates):
            ...
            while successful_sample < 1:
                with gpytorch.settings.prior_mode(True):
                    #print(x.device, device, f'{prior.covar_module.base_kernel.lengthscale=}, {prior.covar_module.base_kernel.lengthscale.device=}')
                    ...
                    try:
                        ...
                        if successful_sample < -100:
                            print(f'Could not sample from prior {i} after {successful_sample} attempts. {e}')
                            raise e
                        continue
                    sample = d.sample() # bs_per_gp_s x T
                    if fix_to_range is None:
                        #for k, v in prior.named_parameters(): print(k,v)
