...
def get_batch(batch_size, seq_len, num_features, get_batch, model, single_eval_pos, epoch, device=default_device, hyperparameters=None, **kwargs):
    ...
    You can train a new prior, based on an old one to only sample from a single level.
    You specify `level_0_model` as a group:index string and the prior will be loaded from the checkpoint.
